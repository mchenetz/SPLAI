# Kubernetes-Native DAEF (CRD-Based)

## Goal
Run DAEF as a Kubernetes control plane where desired state is represented as Custom Resources and reconciled by controllers.

## Core CRDs

1. `DAEFJob` (`daefjobs.daef.io`)
...
2. `DAEFWorker` (`daefworkers.daef.io`)
...
3. `DAEFPolicy` (`daefpolicies.daef.io`)
...
4. `DAEFModelRoute` (`daefmodelroutes.daef.io`)
...
1. Job controller
- Reconciles `DAEFJob` submission lifecycle.
...
- Watches runnable tasks from `DAEFJob`.
- Watches healthy `DAEFWorker` resources.
- Computes score:
...
- Converts worker agent heartbeat stream into `DAEFWorker.status` updates.
- Marks workers stale if heartbeat exceeds threshold.

4. Policy admission webhook
- Validates jobs against matching `DAEFPolicy` before persistence.
- Rejects unsupported model/data combinations and quota violations.

## Runtime Objects and Data Flow

1. User submits `DAEFJob` via API gateway.
2. Job controller ensures DAG exists.
3. Scheduler assigns runnable tasks to `DAEFWorker`.
